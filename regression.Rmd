...
- This presentation is intended to give you a thorough introduction to linear regression

- Before getting started, you'll need to install/load some R packages and data sets
...
## Brief interlude on probability distributions 
$$
E[Y|\underline{X}] = \beta_{0} + X_{1}\beta_{1}+ \cdots + X_{n}\beta_{n} + \epsilon
$$

## Assumptions of GLM's

- The data are assumed to be independently distributed and can be accurately modeled by a distribution from an exponential family (e.g. binomial, Poisson, multinomial, normal,...)

- GLM's do NOT assume a linear relationship between the response variable and the explanatory variables, but it DOES assume linear relationship between the transformed response in terms of the link function and the explanatory variables

- Explanatory variables can be even the power terms or some other nonlinear transformations of the original independent variables

- Simple regression models assume constant variance - but in many cases this assumption does NOT need to be satisfied
...
- In this presentation we'll work with the Ames Housing data set ([De Cock 2011](http://jse.amstat.org/v19n3/decock.pdf)), which is made available by the `AmesHousing` package (also available in the `modeldata` package)

- The data set contains 2930 observations and 80 explanatory variables (23 nominal, 23 ordinal, 14 discrete, and 20 continuous) that are useful for assessing home values

- This data set is an excellent resource for learning about models that we will use throughout this book (Max Kuhn and Julia Silge present a good initial overview of this data set <a target="" href="https://www.tmwr.org/ames.html#ames">here</a>)

- For the purposes of this presentation will be to predict the sale price of a house in Ames, IA based on its other characteristics



## Preparing Our Data

- As previously discussed the initial discovery of relationships is usually done with a training set while a test set is used for evaluating whether the discovered relationships hold
...
- *Simple linear regression* is a very straightforward approach for predicting a <red>quantitative response</red> $Y$ on the basis of a <blue>single predictor variable</blue> $X$

- It assumes that there is approximately a linear relationship between $g(\overline{X}, \overline{\beta})$ and $Y$

- Suppose we wish to model the relationship between `Sale_Price` and the home's total amount of livable square footage `Gr_Liv_Area` - we would express this as:

$$
\text{Sale_Price} = \beta_0 + \beta_1 (\text{Gr_Liv_Area}) + \epsilon \tag{1}
$$

- where: 

    + $\beta_0$ is the intercept
    + $\beta_1$ is the coefficient (slope term) representing the change in the expected value of `Sale_Price` due to changes in `Gr_Liv_Area`
    + $\epsilon$ is a mean-zero random error term $\epsilon \sim NOR(0,\sigma)$

- Objective function: $\text{minimize} \bigg \{ SSE = \sum^n_{i=1} (y_i - \hat{y}_i)^2 \bigg \} \equiv \text{minimize MSE}$

- Recall that these are vector valued quantities, thus we can express equation (1) using matrix notation as:
...
\end{bmatrix} = 
\begin{bmatrix}
1&\text{Gr_Liv_Area}_1\\
1&\text{Gr_Liv_Area}_2\\
\cdots & \cdots\\
1&\text{Gr_Liv_Area}_n
\end{bmatrix}
\begin{bmatrix}
\beta_0\\
\beta_1\\
...
- The `lm` stands for "linear model" and produces the best-fit linear relationship by minimizing the *least squares* criterion  
...
- For an initial assessment of our model we can use `summary(model)`

    + This provides us with a host of information about our model
    + Alternatively, `glance(model)` provides a "tidy" result output
...
Our original formula in Eq. (1) includes $\beta_0$ for our intercept coefficient and $\beta_1$ for our slope coefficient. If we look at our model results (here we use `tidy` to just print out a tidy version of our coefficient results) we see that our model takes the form of
